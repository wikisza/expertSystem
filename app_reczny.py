...
with ruleset('FilmCzySerial'):
    ...
    def Series24(c):
        c.assert_fact({ 'rekomendacja': c.m.rekomendacja, 'wynik': '24 (2001-2010)', 'opis': 'Walka z czasem w stylu thrillera, każdy odcinek to jedna godzina akcji.', 'picture': '24_movie.jpg' })
    ...
    def ThePunisher(c):
        c.assert_fact({ 'rekomendacja': c.m.rekomendacja, 'wynik': 'The Punisher (2017-2019)', 'opis': 'Brutalna opowieść o zemście i sprawiedliwości.' })
    ...
    def Arrow(c):
        c.assert_fact({ 'rekomendacja': c.m.rekomendacja, 'wynik': 'Arrow (2012-2020)', 'opis': 'Bohater walczący z przestępczością w zielonym kapturze.' })

    @when_all((m.preferowanyTyp == 'Serial') & (m.gatunek == 'SciFi') & (m.premiera2015 == 'Przed'))
    def BlackMirror(c):
        c.assert_fact({ 'rekomendacja': c.m.rekomendacja, 'wynik': 'Black Mirror (2011-obecnie)', 'opis': 'Dystopijne historie o wpływie technologii na życie.' })
    ...
    def StrangerThings(c):
        c.assert_fact({ 'rekomendacja': c.m.rekomendacja, 'wynik': 'Stranger Things (2016-obecnie)', 'opis': 'Tajemnice, potwory i dzieci w latach 80.' })
    ...
    def TheExpanse(c):
        c.assert_fact({ 'rekomendacja': c.m.rekomendacja, 'wynik': 'The Expanse (2015-2021)', 'opis': 'Konflikty polityczne w kosmosie.' })

    @when_all((m.preferowanyTyp == 'Serial') & (m.gatunek == 'Fantasy') & (m.premiera2015 == 'Przed'))
    def GameOfThrones(c):
        c.assert_fact({ 'rekomendacja': c.m.rekomendacja, 'wynik': 'Game of Thrones (2011-2019)', 'opis': 'Walka o tron pełna zdrad i intryg.' })
    ...
    def TheWitcher(c):
        c.assert_fact({ 'rekomendacja': c.m.rekomendacja, 'wynik': 'The Witcher (2019-obecnie)', 'opis': 'Wiedźmin Geralt poluje na potwory w fantastycznym świecie.' })
    ...
    def HisDarkMaterials(c):
        c.assert_fact({ 'rekomendacja': c.m.rekomendacja, 'wynik': 'His Dark Materials (2019-2022)', 'opis': 'Magia, nauka i równoległe światy.' })
    ...
    def AmericanHorrorStory(c):
        c.assert_fact({ 'rekomendacja': c.m.rekomendacja, 'wynik': 'American Horror Story (2011-obecnie)', 'opis': 'Antologia horroru z różnymi historiami.' })
    ...
    def TheWalkingDead(c):
        c.assert_fact({ 'rekomendacja': c.m.rekomendacja, 'wynik': 'The Walking Dead (2010-2022)', 'opis': 'Przetrwanie w świecie opanowanym przez zombie.' })
    
    @when_all((m.preferowanyTyp == 'Serial') & (m.gatunek == 'Horror') & (m.premiera2015 == 'Po'))
    def TheHauntingofHillHouse(c):
        c.assert_fact({ 'rekomendacja': c.m.rekomendacja, 'wynik': 'The Haunting of Hill House (2018)', 'opis': 'Rodzinne traumy i nawiedzony dom.' })
    ...
    def TheOffice(c):
        c.assert_fact({ 'rekomendacja': c.m.rekomendacja, 'wynik': 'The Office (US) (2005-2013)', 'opis': 'Komedia o codziennym życiu w biurze.' })
    ...
    def ParksAndRecreation(c):
        c.assert_fact({ 'rekomendacja': c.m.rekomendacja, 'wynik': 'Parks and Recreation (2009-2015)', 'opis': 'Życie urzędników miejskich z humorem.' })

    @when_all((m.preferowanyTyp == 'Serial') & (m.gatunek == 'Komedia') & (m.premiera2010 == 'Po'))
    def Brooklyn99(c):
        c.assert_fact({ 'rekomendacja': c.m.rekomendacja, 'wynik': 'Brooklyn Nine-Nine (2013-2021)', 'opis': 'Zabawy w policyjnym komisariacie.' })
    ...
    def TheCrown(c):
        c.assert_fact({ 'rekomendacja': c.m.rekomendacja, 'wynik': 'The Crown (2016 - obecnie)', 'opis': 'Historia życia królowej Elżbiety II.' })
    ...
    def Succession(c):
        c.assert_fact({ 'rekomendacja': c.m.rekomendacja, 'wynik': 'Succession (2018 - 2023)', 'opis': 'Rodzinne konflikty o przejęcie medialnego imperium.' })

    @when_all((m.preferowanyTyp == 'Serial') & (m.gatunek == 'Dramat') & (m.premiera2010 == 'Przed'))
    def BreakingBad(c):
        c.assert_fact({ 'rekomendacja': c.m.rekomendacja, 'wynik': 'Breaking Bad (2008 - 2013)', 'opis': 'Nauczyciel chemii staje się dilerem narkotyków.' })
    ...
    def Mindhunter(c):
        c.assert_fact({ 'rekomendacja': c.m.rekomendacja, 'wynik': 'Mindhunter (2017 - 2019)', 'opis': 'Profilowanie seryjnych morderców przez FBI.' })
    ...
    def You(c):
        c.assert_fact({ 'rekomendacja': c.m.rekomendacja, 'wynik': 'You (2018 - obecnie)', 'opis': 'Obsesyjna miłość prowadzi do niebezpiecznych działań.' })

    @when_all((m.preferowanyTyp == 'Serial') & (m.gatunek == 'Thiller') & (m.premiera2015 == 'Przed'))
    def TrueDetective(c):
        c.assert_fact({ 'rekomendacja': c.m.rekomendacja, 'wynik': 'True Detective (2014 - obecnie)', 'opis': 'Mroczne kryminały w antologicznej formie.' })
    ...
    def RickAndMorty(c):
        c.assert_fact({ 'rekomendacja': c.m.rekomendacja, 'wynik': 'Rick and Morty (2013-obecnie)', 'opis': 'Szalone przygody dziadka naukowca i wnuka. ' })
    ...
    def BoJackHorseman(c):
        c.assert_fact({ 'rekomendacja': c.m.rekomendacja, 'wynik': 'BoJack Horseman (2014 - 2020)', 'opis': 'Animowany dramat o upadłej gwieździe.' })

    @when_all((m.preferowanyTyp == 'Serial') & (m.gatunek == 'Animacja') & (m.premiera2010 == 'Przed'))
    def Avatar(c):
        c.assert_fact({ 'rekomendacja': c.m.rekomendacja, 'wynik': 'Avatar: The Last Airbender (2005-2008)', 'opis': 'Epicka podróż młodego Avatara.' })
    ...
    def Bridgerton(c):
        c.assert_fact({ 'rekomendacja': c.m.rekomendacja, 'wynik': 'Bridgerton (2020-obecnie)', 'opis': 'Romansy i intrygi w XIX-wiecznym Londynie.' })
    ...
    def NormalPeople(c):
        c.assert_fact({ 'rekomendacja': c.m.rekomendacja, 'wynik': 'Normal People (2020)', 'opis': 'Miłosna relacja pełna emocji i komplikacji.' })

    @when_all((m.preferowanyTyp == 'Serial') & (m.gatunek == 'Romans') & (m.premiera2015 == 'Przed'))
    def Outlander(c):
        c.assert_fact({ 'rekomendacja': c.m.rekomendacja, 'wynik': 'Outlander (2014-obecnie)', 'opis': 'Miłość i przygody w podróżach przez czas.' })
    ...
    def Narcos(c):
        c.assert_fact({ 'rekomendacja': c.m.rekomendacja, 'wynik': 'Narcos (2015-2017)', 'opis': 'Historia Pabla Escobara i walki z narkobiznesem.' })
    ...
    def BetterCallSaul(c):
        c.assert_fact({ 'rekomendacja': c.m.rekomendacja, 'wynik': 'Better Call Saul (2015-2022)', 'opis': 'Prequel o prawniczych początkach Saula Goodmana.' })

    @when_all((m.preferowanyTyp == 'Serial') & (m.gatunek == 'Kryminal') & (m.premiera2015 == 'Przed'))
    def Sherlock(c):
        c.assert_fact({ 'rekomendacja': c.m.rekomendacja, 'wynik': 'Sherlock (2010-2017)', 'opis': 'Współczesna interpretacja genialnego detektywa.' })

    ##   filmy

    @when_all((m.preferowanyTyp == 'Film') & (m.gatunek == 'Akcja') & (m.premiera2010 == 'Przed'))
    def DieHard(c):
        c.assert_fact({ 'rekomendacja': c.m.rekomendacja, 'wynik': 'Die hard (1998)', 'opis': 'Klasyczny film akcji z pojedynkiem w wieżowcu.' })
    ...
    def MadMax(c):
        c.assert_fact({ 'rekomendacja': c.m.rekomendacja, 'wynik': 'Mad Max: Fury Road(2015)', 'opis': 'Postapokaliptyczna pogoń przez pustynię.' })
    ...
    def JohnWick(c):
        c.assert_fact({ 'rekomendacja': c.m.rekomendacja, 'wynik': 'John Wick (2014)', 'opis': 'Zemsta i brawurowe sceny walki.' })

    # SciFi
    @when_all((m.preferowanyTyp == 'Film') & (m.gatunek == 'SciFi') & (m.premiera2011 == 'Przed'))
    def Inception(c):
        c.assert_fact({ 'rekomendacja': c.m.rekomendacja, 'wynik': 'Inception (2010)', 'opis': 'Sny w snach i kradzież tajemnic.' })
    ...
    def BladeRunner(c):
        c.assert_fact({ 'rekomendacja': c.m.rekomendacja, 'wynik': 'Blade Runner 2049 (2017)', 'opis': 'Filozoficzna opowieść o androidach.' })
    ...
    def Interstellar(c):
        c.assert_fact({ 'rekomendacja': c.m.rekomendacja, 'wynik': 'Interstellar (2014)', 'opis': 'Podróż w kosmosie w poszukiwaniu nadziei.' })

    # fantasy
    @when_all((m.preferowanyTyp == 'Film') & (m.gatunek == 'Fantasy') & (m.premiera2005 == 'Po'))
    def PansLabyrinth(c):
        c.assert_fact({ 'rekomendacja': c.m.rekomendacja, 'wynik': 'Pans Labyrinth (2006)', 'opis': 'Mroczna baśń w Hiszpanii czasów wojny.' })
    ...
    def TheLordOfTheRings(c):
        c.assert_fact({ 'rekomendacja': c.m.rekomendacja, 'wynik': 'The Lord of the Rings (2001)', 'opis': 'Walka o zniszczenie pierścienia w Śródziemiu.' })
    ...
    def HarryPotter(c):
        c.assert_fact({ 'rekomendacja': c.m.rekomendacja, 'wynik': 'Harry Potter and the Sorcerers Stone (2001)', 'opis': 'Początek przygody w świecie magii.' })
    ...
    def Hereditary(c):
        c.assert_fact({ 'rekomendacja': c.m.rekomendacja, 'wynik': 'Hereditary (2018)', 'opis': 'Rodzinna tragedia w atmosferze grozy.' })
    ...
    def AQuietPlace(c):
        c.assert_fact({ 'rekomendacja': c.m.rekomendacja, 'wynik': 'A Quiet Place (2018)', 'opis': 'Przetrwanie w świecie ciszy.' })
    
    @when_all((m.preferowanyTyp == 'Film') & (m.gatunek == 'Horror') & (m.premiera2010 == 'Przed'))
    def TheExorcist(c):
        c.assert_fact({ 'rekomendacja': c.m.rekomendacja, 'wynik': 'The Exorcist (1973)', 'opis': 'Kultowy horror o opętaniu.' })
    ...
    def GroundhogDay(c):
        c.assert_fact({ 'rekomendacja': c.m.rekomendacja, 'wynik': 'Groundhog Day (1993)', 'opis': 'Powtarzający się dzień zmienia życie.' })
    ...
    def Superbad(c):
        c.assert_fact({ 'rekomendacja': c.m.rekomendacja, 'wynik': 'Superbad (2007)', 'opis': 'Szalona komedia o nastoletnich przygodach.' })

    @when_all((m.preferowanyTyp == 'Film') & (m.gatunek == 'Komedia') & (m.premiera2010 == 'Po'))
    def TheGrandBudapestHotel(c):
        c.assert_fact({ 'rekomendacja': c.m.rekomendacja, 'wynik': 'The Grand Budapest Hotel (2014)', 'opis': 'Barwna komedia o ekscentrycznym hotelu.' })

    #dramat
    @when_all((m.preferowanyTyp == 'Film') & (m.gatunek == 'Dramat') & (m.premiera1960 == 'Po') & (m.opisDramatF == 'Rodzinne intrygi, mafia, klasyczne dialogi.'))
    def TheGodfather(c):
        c.assert_fact({ 'rekomendacja': c.m.rekomendacja, 'wynik': 'The Godfather (1972)', 'opis': 'Klasyka o mafijnym imperium.' })
    ...
    def ShindlersList(c):
        c.assert_fact({ 'rekomendacja': c.m.rekomendacja, 'wynik': 'Shindlers List (1993)', 'opis': 'Poruszająca opowieść o Holokauście.' })

    @when_all((m.preferowanyTyp == 'Film') & (m.gatunek == 'Dramat') & (m.premiera1960 == 'Przed'))
    def Movie12AngryMan(c):
        c.assert_fact({ 'rekomendacja': c.m.rekomendacja, 'wynik': '12 Angry Man (1957)', 'opis': 'Dramat sądowy w pokoju narad.' })
    ...
    def Se7en(c):
        c.assert_fact({ 'rekomendacja': c.m.rekomendacja, 'wynik': 'Se7en (1995)', 'opis': 'Mroczny thriller o seryjnym mordercy.' })
    ...
    def TheSilenceOfTheLambs(c):
        c.assert_fact({ 'rekomendacja': c.m.rekomendacja, 'wynik': 'The Silence of the Lambs (1991)', 'opis': 'Psychologiczna gra z Hannibalem Lecterem.' })

    @when_all((m.preferowanyTyp == 'Film') & (m.gatunek == 'Thiller') & (m.premiera2000 == 'Po'))
    def GoneGirl(c):
        c.assert_fact({ 'rekomendacja': c.m.rekomendacja, 'wynik': 'Gone Girl (2014)', 'opis': 'Zniknięcie, które kryje mroczne sekrety.' })
    ...
    def SpiritedAway(c):
        c.assert_fact({ 'rekomendacja': c.m.rekomendacja, 'wynik': 'Spirited Away (2001)', 'opis': 'Magiczna podróż dziewczynki w świat duchów.' })
    ...
    def Shrek(c):
        c.assert_fact({ 'rekomendacja': c.m.rekomendacja, 'wynik': 'Shrek (2001)', 'opis': 'Baśń na opak z zabawnym ogram.' })

    @when_all((m.preferowanyTyp == 'Film') & (m.gatunek == 'Animacja') & (m.premiera2001 == 'Przed'))
    def ToyStory(c):
        c.assert_fact({ 'rekomendacja': c.m.rekomendacja, 'wynik': 'Toy Story (1995)', 'opis': 'Ożywione zabawki w wielkiej przygodzie.' })
    ...
    def PridePrejudice(c):
        c.assert_fact({ 'rekomendacja': c.m.rekomendacja, 'wynik': 'Pride & Prejudice (2005)', 'opis': 'Klasyczna historia miłości i uprzedzeń.' })
    ...
    def TheNotebook(c):
        c.assert_fact({ 'rekomendacja': c.m.rekomendacja, 'wynik': 'The Notebook (2004)', 'opis': 'Wzruszająca opowieść o wielkiej miłości.' })
    ...
    def PulpFiction(c):
        c.assert_fact({ 'rekomendacja': c.m.rekomendacja, 'wynik': 'Pulp Fiction (1994)', 'opis': 'Kultowy, niekonwencjonalny film gangsterski.' })
    ...
    def Heat(c):
        c.assert_fact({ 'rekomendacja': c.m.rekomendacja, 'wynik': 'Heat (1995)', 'opis': 'Pojedynek policjanta z geniuszem zbrodni.' })

    @when_all((m.preferowanyTyp == 'Film') & (m.gatunek == 'Kryminal') & (m.premiera2000 == 'Po'))
    def TheDeparted(c):
        c.assert_fact({ 'rekomendacja': c.m.rekomendacja, 'wynik': 'The Departed (2006)', 'opis': 'Intrygi i zdrady w policji i mafii.' })



    @when_all(+m.rekomendacja)
    def output(c):
        print('Propozycja: {0}.{1} Opis filmu: {2}'.format(c.m.rekomendacja, c.m.wynik, c.m.opis))


post('FilmCzySerial',{'rekomendacja': '1', 'preferowanyTyp': 'Film', 'gatunek': 'Komedia', 'premiera2010': 'Po' })
post('FilmCzySerial',{'rekomendacja': '2', 'preferowanyTyp': 'Serial', 'gatunek': 'Kryminal', 'premiera2015': 'Po', 'opisKryminalS': 'Historia prawnika w swiecie przestepczym, prequel Breaking Bad, moralne dylematy.'})
